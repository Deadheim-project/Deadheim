...
namespace BetterWardsPlugin
{
    ...
    public static class AssetHelper
    {
        ...
        {
            Plugin.Log.LogInfo("LoadAssetBundle Inicio");
            ...
            Plugin.Log.LogInfo("LoadAssetBundle Fim");
        ...
        {
            Plugin.Log.LogInfo("RegisterPrefab Inicio");

            Prefabs.Add(prefab);
            GameObject.DontDestroyOnLoad(prefab);

            Plugin.Log.LogInfo("RegisterPrefab Fim");
        }

        public static void RegisterRecipe(RecipeStub rs)
        {
            Plugin.Log.LogInfo("RegisterRecipe Inicio");

            RecipeStubs.Add(rs);

            Plugin.Log.LogInfo("RegisterRecipe Fim");

        }

        public static void RegisterStatusEffect<T>(string name) where T : StatusEffect
        {
            Plugin.Log.LogInfo("RegisterStatusEffect Inicio");

            StatusEffects.Add(new KeyValuePair<string, StatusEffect>(name, ScriptableObject.CreateInstance<T>()));

            Plugin.Log.LogInfo("RegisterStatusEffect Fim");
        }

        public static RecipeStub GetRecipeStub(string itemName)
        {
            Plugin.Log.LogInfo("GetRecipeStub");
        ...
        {
            Plugin.Log.LogInfo("BuildRecipe Inicio");
        ...
        {
            Plugin.Log.LogInfo("PopulateObjectDB Inicio");
        ...
        {
            Plugin.Log.LogInfo("ObjectDB Awake Inicio");
        ...
        {
            Plugin.Log.LogInfo("ObjectDB CopyOtherDB Inicio");
        ...
        public static void AwakePostfix(ZNetScene __instance)
        {
            ...
            Plugin.Log.LogInfo("ZNetScene Awake Inicio");
        ...
        public static void AwakePostfix(Chat __instance)
        {
            Plugin.Log.LogInfo("Chat Awake Inicio");

            Plugin.Log.LogInfo("Chat Awake fim");
        }

        [HarmonyPostfix]
        [HarmonyPatch(typeof(Chat), "OnNewChatMessage")]
        public static void OnNewChatMessagePostfix(GameObject go,
            long senderID,
            Vector3 pos,
            Talker.Type type,
            string user,
